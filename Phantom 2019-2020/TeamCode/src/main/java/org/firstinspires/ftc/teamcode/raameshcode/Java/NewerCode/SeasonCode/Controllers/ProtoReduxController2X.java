...
public class ProtoReduxController2X extends LinearOpMode {
    ...
    boolean debug=true;
    public ProtoReduxController2X(LinearOpMode lin){
        linearOpMode = lin;
    ...
    public void complexDrive(MecanumConfig robot, double angle, double distance, double power){
        ...
        robot.backRight.setPower((Math.cos(Math.toRadians(angle)) * power));
        robot.backLeft.setPower((Math.sin(Math.toRadians(angle)) * power));
        while (methods.getTrackingDistance() < distance);
        robot.stop();
    }
    ...
    @Deprecated
    public static void ΔxΔyDrive (LinearOpMode l,double Δx,double Δy,double power) {
        MecanumConfig robot23 = new MecanumConfig();
        double diaOne = Δy+Δx;
        double diaTwo = Δy-Δx;
