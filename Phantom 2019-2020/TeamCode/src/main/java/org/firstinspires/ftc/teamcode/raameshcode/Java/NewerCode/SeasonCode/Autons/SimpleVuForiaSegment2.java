...
@Autonomous(name = "NEWNESSS")
public class SimpleVuForiaSegment2 extends LinearOpMode {
...
//
        ...
        .75);
*//*
     telemetry.addData("imu calib status", robot.i
        mu.getCalibrationStatus().toString());
        telemetry.update();
...
*/
        ...
        reduxController.middleDrive(robot,0,0.3);
        while(robot.sensorRange.getDistance(DistanceUnit.INCH) < 20&&!isStopRequested());
        timeTracking.reset();
        targetsSkyStone.activate();
        while (!isStopRequested() && timeTracking.seconds() < 5) {
            ...
            if (targetVisible) {
                ...
                if (translation.get(1) / mmPerInch < 5 && translation.get(1) / mmPerInch > -5) {
                    robot.stop();
                    break;
                }
            } else {
        ...
        if (targetVisible) {
            VectorF translation = lastLocation.getTranslation();
            double vars = translation.get(1);
            if (vars / mmPerInch < 0) {
                robot.stop();
                reduxController.middleDrive(robot, -13.5, 0.4);
                sleep(550);
            }else {
                if (vars / mmPerInch > 0) {
                    reduxController.middleDrive(robot, 13.5, 0.4);
        ...
        } /*if (targetVisible)*/else {
            reduxController.middleDrive(robot, -30.5, 0.4);
        }
        sleep(2000);/*else for if (targetVisible)*/
    }
}
